...
describe("loadConfig", () => {
  it("uses local Ollama defaults without requiring a configured model", () => {
    ...
    expect(config.backups.directory).toContain(".arc/backups");
    expect(config.security).toEqual({ auditRetentionDays: 90, permissionProfile: "review" });
  ...
  it("normalizes the Ollama base URL and accepts a configured model", () => {
    const config = loadConfig({
      ...
      ARC_BACKUP_DIRECTORY: "/tmp/arc-backups",
      ARC_PERMISSION_PROFILE: "read_only",
      ARC_SECURITY_AUDIT_RETENTION_DAYS: "30",
    ...
    expect(config.backups).toEqual({ directory: "/tmp/arc-backups" });
    expect(config.security).toEqual({ auditRetentionDays: 30, permissionProfile: "read_only" });
    expect(config.chatContext).toEqual({
  ...
  it("expands the home directory for backups", () => {
    expect(loadConfig({ ARC_BACKUP_DIRECTORY: "~/.arc/test-backups" }).backups.directory).not.toContain("/~");
  });
});
